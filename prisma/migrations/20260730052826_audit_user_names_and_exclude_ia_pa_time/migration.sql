-- ============================================================
-- Two changes to the SalesOrder audit trigger:
--
-- 1. Resolve issueAssignedUserId / packingAssignedUserId to the
--    actual account name (User.name) for display, the same way
--    transporterId already resolves to the Transporter name.
--    Without this, the Audit History dialog showed raw numeric
--    user IDs instead of a readable name.
--
-- 2. Stop tracking IA_Time / PA_Time as their own change entries.
--    They change automatically as a side effect whenever
--    issueAssignedUserId / packingAssignedUserId change, so they
--    always appeared as a second, redundant, unreadable entry
--    right next to the actual "Issue/Packing Assigned User"
--    change. Excluded the same way updatedAt/UpdatedDate already
--    are.
-- ============================================================
                ...
                jsonb_strip_nulls(
                    jsonb_build_object(
                        ...
                        CASE
                            ...
                            WHEN old_data.key IN ('issueAssignedUserId', 'packingAssignedUserId')
                                 AND old_data.value IS NOT NULL
                                 AND old_data.value <> 'null'::jsonb
                            THEN (
                                SELECT to_jsonb(u."name")
                                FROM "User" u
                                WHERE u."id" =
                                    (old_data.value #>> '{}')::INTEGER
                                LIMIT 1
                        ...
                        CASE
                            ...
                            WHEN new_data.key IN ('issueAssignedUserId', 'packingAssignedUserId')
                                 AND new_data.value IS NOT NULL
                                 AND new_data.value <> 'null'::jsonb
                            THEN (
                                SELECT to_jsonb(u."name")
