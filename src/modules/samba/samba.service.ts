...
export class SambaService {
  ...
  async listFiles(folder: string) {
    ...
    try {
      ...
      // 3. Cross-reference ERROR files with the Database
        ...
        const soNumbers = mappedFiles.map((f) =>
          f.filename.replace(/\.[^/.]+$/, ''),
        );
      ...
      this.logger.warn(
        `Could not read directory ${targetDir}. It might be empty or missing.`,
      );
      return [];
    }
  }
  ...
  async downloadFiles(folder: string, filenames: string[], res: Response) {
    ...
    if (filenames.length === 1) {
      ...
      res.setHeader(
        'Content-Disposition',
        `attachment; filename="${folder.toUpperCase()}_FILES.zip"`,
      );
  ...
  async downloadFailedErpData(
    ...
    if (!orders.length) {
      throw new NotFoundException('No matching sales orders found.');
    }
    ...
    for (const order of orders) {
      if (!order.saleOrderNumber || !order.outboundDelivery) {
        missing.push(`Order ${order.id}`);
        continue;
      }

      const expectedFileName = `${order.saleOrderNumber}_${order.outboundDelivery}.xlsx`;

      if (existingFileNames.has(expectedFileName)) {
        filesToDownload.push(expectedFileName);
      } else {
        missing.push(`${order.saleOrderNumber}_${order.outboundDelivery}`);
      }
    }

    if (filesToDownload.length === 0) {
      throw new NotFoundException(
        'No failed ERP files found for the selected orders in the ERROR folder.',
      );
    }
